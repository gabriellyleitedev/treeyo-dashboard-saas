...
export default function BuscaInteligente({ isOpen, onClose, navigate, rotasDoSistema = [] }) {
  ...
  // Abas de sugestão que você pediu
  const sugestoes = [
  ...
  // LÓGICA DE VARREDURA TOTAL (IGUAL AO UBER)
  const results = useMemo(() => {
  if (!query.trim()) return [];
  const lowerQuery = query.toLowerCase();

  return rotasDoSistema.filter((item) => {
    // Se o item tem a 'searchData' que criamos no Layout, usa ela. 
    // Se não, usa o nome (para as abas do menu).
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          {/* Backdrop com o Blur Pesado que você pediu */}
                ...
                <div className="space-y-4">
                  {results.map((item, i) => (
                    <div 
                      ...
                      className="flex items-center justify-between p-5 rounded-2xl hover:bg-white/[0.08] cursor-pointer group transition-all border border-transparent hover:border-white/10"
                    >
                      <div className="flex items-center gap-5">
                        <div className="w-12 h-12 flex items-center justify-center rounded-2xl bg-neutral-900 text-neutral-400 group-hover:text-[#4dff00] transition-all">
                      ...
                      <ArrowRight size={18} className="text-neutral-800 group-hover:text-[#4dff00] transition-transform group-hover:translate-x-2" />
                    </div>
                  ))}
                  {results.length === 0 && (
                      ...
                      <div className="space-y-1">
                        {history.map((h, i) => (
                          <div key={i} onClick={() => setQuery(h)} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-white/5 cursor-pointer text-neutral-400 hover:text-white transition-all group">
                            <History size={16} className="text-neutral-700 group-hover:text-[#4dff00]" />
